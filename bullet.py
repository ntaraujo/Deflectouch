...
class Bullet(Image):
    ...
    def check_deflector_collision(self, deflector):
        ...
        intersection = Vector.line_intersection(bullet_position, bullet_position + bullet_direction, deflector_point1,
                                                deflector_point2)
        ...
        distance = abs(sin(radians(bullet_direction.angle(deflector_vector)) % (pi / 2))) * Vector(
            intersection - bullet_position).length()
    ...
    def callback_pos(self, instance, pos):
        ...
        # and therefore a callback is raised then)
