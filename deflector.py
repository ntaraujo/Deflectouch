...
class Deflector(Scatter):
    ...
    def __init__(self, **kwargs):
        ...
        with self.canvas.before:
            ...
            self.deflector_line = Line(
                points=(self.touch1.x, self.touch1.y - 1, self.touch1.x + self.length, self.touch1.y - 1))
            self.deflector_line2 = Line(
                points=(self.touch1.x, self.touch1.y + 1, self.touch1.x + self.length, self.touch1.y + 1))
    ...
    def size_callback(self, instance, size):
        ...
        # feedback to the stock_bar: reducing of the deflector material stock:
        # self.length = Vector(self.touch1.pos).distance(self.touch2.pos)
        self.length = self.length_origin * self.scale
        try:
            self.parent.parent.stock_bar.recalculate_stock()
        except Exception:
            return
        # get the current stock from the root widget:
        current_stock = self.parent.parent.stock_bar.width
    ...
    def collide_point(self, x, y):
        ...
        # therefore i can control when a touch will be dispatched from here.
        point1_parent = self.to_parent(self.point1.center[0], self.point1.center[1])
        point2_parent = self.to_parent(self.point2.center[0], self.point2.center[1])

        return min(point1_parent[0], point2_parent[0]) - GRAB_RADIUS <= x <= max(point1_parent[0],
                                                                                 point2_parent[0]) + GRAB_RADIUS \
               and min(point1_parent[1], point2_parent[1]) - GRAB_RADIUS <= y <= max(point1_parent[1],
                                                                                     point2_parent[1]) + GRAB_RADIUS

    def collide_grab_point(self, x, y):
        point1_parent = self.to_parent(self.point1.center[0], self.point1.center[1])
        point2_parent = self.to_parent(self.point2.center[0], self.point2.center[1])

        return point1_parent[0] - GRAB_RADIUS <= x <= point1_parent[0] + GRAB_RADIUS and point1_parent[
            1] - GRAB_RADIUS <= y <= point1_parent[1] + GRAB_RADIUS \
               or point2_parent[0] - GRAB_RADIUS <= x <= point2_parent[0] + GRAB_RADIUS and point2_parent[
                   1] - GRAB_RADIUS <= y <= point2_parent[1] + GRAB_RADIUS
    ...
    def on_touch_up(self, touch):
        ...
        if self.length < MIN_DEFLECTOR_LENGTH and self.parent is not None:
            self.parent.delete_deflector(self)
            return True

        if self.parent is not None and self.collide_grab_point(*touch.pos):
