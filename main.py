...
class DeflectouchWidget(Widget):
        ...
        for row in range(5):
            for column in range(8):
                ...
                if enable_next_row:
                    ...
                    if self.app.config.get('GamePlay', 'Levels')[row * 8 + column] == '1':
    ...
    def accomplished_animation_complete(self, animation, widget):
        ...
        if self.level != 40:
            if self.level % 8 == 0:
                ...
                Popup(title='New levels unlocked!', content=Label(text='Next 8 levels unlocked!', font_size=18),
    ...
    def load_level(self, level):
        ...
        try:
            level_image = kivy.core.image.Image.load(self.app.directory + '/levels/level%02d.png' % level,
                                               keep_data=True)
        except Exception as e:
            error_text = 'Unable to load Level %d!\n\nReason: %s' % (level, e)
            Popup(title='Level loading error:', content=Label(text=error_text, font_size=18),
                  size_hint=(0.3, 0.2)).open()
            return
        ...
        if self.stock_bar is not None:
            self.remove_widget(self.stock_bar)
        self.max_stock = 0
...
class Deflectouch(App):
    ...
    def build(self):
        ...
        # create the root widget and give it a reference of the application instance (so it can access the
        # application settings)
        self.deflectouch_widget = DeflectouchWidget(app=self)
        self.root = self.deflectouch_widget
        ...
        for counter, char in enumerate(self.config.get('GamePlay', 'Levels')):
            ...
            if char == '0':
                self.deflectouch_widget.load_level(counter + 1)
                level_opened = True
                break

        # if all levels were completed, just open the last one.
        if not level_opened:
            self.deflectouch_widget.load_level(40)
